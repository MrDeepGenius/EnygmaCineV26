...
async function fetchSheet(key: keyof typeof SHEET_URLS): Promise<Record<string, string>[]> {
  ...
  logger.info({ sheet: key }, "Fetching Google Sheet");
  const res = await fetch(SHEET_URLS[key], { redirect: "follow" });
  if (!res.ok) {
    throw new Error(`Failed to fetch sheet ${key}: ${res.status}`);
  }
...
export async function getHomeContent(profile?: string): Promise<{
  ...
  const [moviesRaw, { seriesList }, { animeList }] = await Promise.all([
    fetchSheet("movies"),
    buildSeriesAndEpisodes(),
    buildAnimeAndEpisodes(),
  ]);

  let allMovies: Movie[] = moviesRaw
    .filter((r) => r["id_unico"] && r["titulo"])
    .map((r) => ({
      id: r["id_unico"],
      titulo: r["titulo"] || "",
  ...
  const bannerRaw = allMovies.filter((m) => m.enBanner && m.backdropUrl).slice(0, 8);
  const bannerLogos = await Promise.all(
    bannerRaw.map((m) => getLogoUrl(m.id, "movie").catch(() => null))
  );
  const banner = bannerRaw.map((m, i) => ({ ...m, logoUrl: bannerLogos[i] ?? null }));
  ...
  return {
    banner: banner.length > 0 ? banner : latestMovies.slice(0, 5),
    top10,
    trending: trending2,
