...
export default function Movies() {
  ...
  const allItems: Movie[] = data?.items || [];
        ...
        <div className="flex flex-col gap-3 mb-5 md:mb-8">
          ...
          <div className="flex gap-1.5 overflow-x-auto pb-1 scrollbar-hide">
            <button
              ...
              className={`flex-shrink-0 px-3 py-1 rounded-full text-xs font-bold transition-all ${!genre ? "bg-[#A855F7] text-white shadow-lg shadow-[#A855F7]/30" : "bg-white/8 text-white/50 hover:text-white border border-white/10"}`}
            >
              Todos
            </button>
            {GENRES.map(g => (
              <button
                key={g}
                onClick={() => { setGenre(g); setDisplayLimit(PAGE_SIZE); }}
                className={`flex-shrink-0 px-3 py-1 rounded-full text-xs font-bold transition-all ${genre === g ? "bg-[#A855F7] text-white shadow-lg shadow-[#A855F7]/30" : "bg-white/8 text-white/50 hover:text-white border border-white/10"}`}
              >
                {g}
              </button>
            ))}
