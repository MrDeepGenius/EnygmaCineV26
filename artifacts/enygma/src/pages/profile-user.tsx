...
const PROFILE_DATA = {
  senor: { name: "Sr. Enigma", avatar: "/avatar-senor.png", color: "#A855F7", glow: "#A855F7" },
  ...
  return (
    <Layout>
      <div className="max-w-screen-lg mx-auto px-4 md:px-8 py-6 pb-10">
        ...
        {/* ── Favoritos ── */}
        <Section
          icon={<Heart className="w-4 h-4 text-[#A855F7] fill-[#A855F7]" />}
          title="Mis Favoritos"
          color="#A855F7"
          linkHref="/mylist"
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2.5">
              {favorites.slice(0, 12).map((fav) => (
                <Link key={fav.id} href={`/detail/${fav.tipo}/${fav.id}`}>
                  <div className="aspect-[2/3] rounded-lg overflow-hidden bg-zinc-900 cursor-pointer hover:scale-105 transition-transform border border-white/5 hover:border-[#A855F7]/40">
                    {fav.posterUrl ? (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    <Link href={`/watch/${item.tipo}/${item.id}`}>
                      <div className="w-7 h-7 rounded-full bg-white/10 group-hover:bg-[#A855F7] flex items-center justify-center transition-colors">
