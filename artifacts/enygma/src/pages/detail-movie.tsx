...
import {
  Play, Heart, Star, ChevronLeft, Calendar, PlayCircle, Clock,
  Globe, Mic2, Subtitles, Film, User, DollarSign, Award,
  ExternalLink, ChevronRight, Video,
} from "lucide-react";
import { useGetMovie, getGetMovieQueryKey, useGetTmdbDetails, getGetTmdbDetailsQueryKey } from "@workspace/api-client-react";
    ...
    <Layout>
      ...
      <div className="relative w-full h-[62vh] md:h-[80vh] -mt-14 md:-mt-16">
        ...
        <div className="absolute top-16 md:top-20 left-4 md:left-10 z-10">
          <Link href="/movies" className="inline-flex items-center gap-1.5 text-white/60 hover:text-white text-sm font-medium transition-colors bg-black/40 backdrop-blur-sm px-3 py-1.5 rounded-full border border-white/10">
            <ChevronLeft className="w-4 h-4" /> Películas
          </Link>
        </div>
            ...
            <div className="flex flex-wrap items-center gap-2 md:gap-3 text-sm text-white/70 mb-4 font-medium">
              ...
              {movie.año && (
                <span className="flex items-center gap-1">
                  <Calendar className="w-3.5 h-3.5 text-white/40" /> {movie.año}
                </span>
              )}
              {tmdb?.runtime && (
                <span className="flex items-center gap-1">
              ...
              {movie.esVip && (
                <span className="bg-[#A855F7]/20 text-[#A855F7] border border-[#A855F7]/50 px-2 py-0.5 rounded text-xs font-bold uppercase">VIP</span>
              )}
            </div>

            {/* Synopsis short */}
            {movie.sinopsis && (
              <p className="text-white/75 text-sm md:text-base leading-relaxed max-w-xl mb-5 line-clamp-2">
                {movie.sinopsis}
              </p>
            )}

            {/* Director pill in hero */}
            {tmdb?.director && (
              <p className="text-white/40 text-xs mb-5">
                <span className="text-white/25">Dir. </span>
                <span className="text-white/65 font-semibold">{tmdb.director}</span>
              </p>
            )}
      ...
      <div className="max-w-screen-2xl mx-auto px-4 md:px-10 lg:px-16 xl:px-20 py-10 space-y-14">
        ...
        <section className="flex flex-col md:flex-row gap-8 md:gap-10">
          ...
          {movie.posterUrl && (
            <div className="flex-shrink-0 w-36 md:w-48 lg:w-56 self-start">
              <div className="rounded-xl overflow-hidden shadow-2xl ring-1 ring-white/10">
                <img src={movie.posterUrl} alt={movie.titulo} className="w-full object-cover" />
              </div>
            </div>
          )}
          ...
          <div className="flex-1 space-y-7">
            ...
            <div className="flex flex-wrap gap-3 items-stretch">
              ...
              {tmdb?.voteAverage && (
                <div className="flex flex-col items-center justify-center bg-zinc-900 border border-yellow-500/30 rounded-xl px-5 py-3 min-w-[80px]">
                  <div className="flex items-center gap-1 mb-0.5">
                    ...
                    <span className="text-yellow-400 font-black text-xl leading-none">{tmdb.voteAverage.toFixed(1)}</span>
                  </div>
              ...
              {score !== null && (parseFloat(String(score)) >= 7.5) && (
            ...
            {movie.sinopsis && (
              <div>
                ...
                <p className="text-white/80 leading-relaxed text-sm md:text-base">{movie.sinopsis}</p>
              </div>
            )}
            ...
            {/* Streaming */}
            <StreamingPlatforms contentId={movie.id} count={5} />
          </div>
        </section>

        {/* ── METADATA GRID ── */}
        {(tmdb?.director || tmdb?.writers?.length || tmdb?.spokenLanguages?.length || tmdb?.productionCountries?.length || tmdb?.status || tmdb?.budget || tmdb?.revenue) && (
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-10 gap-y-5">
              ...
              {tmdb?.director && (
                <div className="flex gap-3">
                  ...
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-white/30 mb-0.5">Director</p>
                    <p className="text-white/80 text-sm font-semibold">{tmdb.director}</p>
                  </div>
                </div>
              )}
              ...
              {tmdb?.writers && tmdb.writers.length > 0 && (
                <div className="flex gap-3">
                  ...
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-white/30 mb-0.5">Guion</p>
                    <p className="text-white/80 text-sm">{tmdb.writers.join(", ")}</p>
                  </div>
                </div>
              )}
              ...
              {tmdb?.status && (
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-white/80 text-sm">{tmdb.status}</p>
                  </div>
                </div>
              )}
              ...
              {tmdb?.budget && (
                <div className="flex gap-3">
                  ...
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-white/30 mb-0.5">Presupuesto</p>
                    <p className="text-white/80 text-sm">{formatMoney(tmdb.budget)}</p>
                  </div>
                </div>
              )}
              ...
              {tmdb?.revenue && (
                <div className="flex gap-3">
                  ...
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-white/30 mb-0.5">Recaudacion</p>
                    <p className="text-green-400 text-sm font-semibold">{formatMoney(tmdb.revenue)}</p>
                  </div>
                </div>
              )}
        ...
        {movie.actores && cast.length === 0 && (
          <section>
            <h2 className="text-[11px] font-bold font-display uppercase tracking-[0.2em] text-white/30 mb-3">Reparto</h2>
            <p className="text-white/60 leading-relaxed">{movie.actores}</p>
          </section>
        )}
